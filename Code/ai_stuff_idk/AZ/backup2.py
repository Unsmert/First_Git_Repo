...
class Alpha_MCTS:
    ...
    def __init__(self, game, args, model):
        ...
        self.args = args
        self.model = model
        self.root = Node(self.game, self.args, self.game.get_initial_state(), visit_count= 1)

    @torch.no_grad()
    # Actually runs the MCTS here
    def search(self, state, prev_action):
        # Makes a new root node with the current state
        self.update_root(prev_action= prev_action)
    ...
    def update_root(self, prev_action):
        if prev_action == None:
            self.root = Node(self.game, self.args, self.game.get_initial_state(), visit_count= 1)
            return
        else:
            for child in self.root.children:
                if child.action_taken == prev_action:
                    self.root = child
                    self.root.parent = None
                    return
            self.root = Node(self.game, self.args, self.game.change_perspective(self.game.get_next_state(self.root.state, prev_action, 1), 1), visit_count= 1)

class Alpha_MCTS_Parallel:
    def __init__(self, game, args, model):
        self.game = game
        self.args = args
        self.model = model
    ...
    @torch.no_grad()
    def search(self, states, spGames, prev_actions):
        policy, _ = self.model(
        ...
        for i, spg in enumerate(spGames):
            ...
            spg.update_root(prev_actions[i], spg_policy)
...
class AlphaZeroParallel:
    def __init__(self, model, optimizer, game, args):
        ...
        self.game = game
        self.args = args
        self.mcts = Alpha_MCTS_Parallel(game, args, model)
        
    def selfPlay(self):
        return_memory = []
        player = 1
        spGames = [SPG(self.game, self.args) for spg in range(self.args['num_parallel_games'])]
        prev_actions = [None for _ in range(len(spGames))]
        
        while len(spGames) > 0:
            states = np.stack([spg.state for spg in spGames])
            neutral_states = self.game.change_perspective(states, player)
            self.mcts.search(neutral_states, spGames, prev_actions)
            prev_actions = []
    ...
    def learn(self):
        total_memory = []
        for iteration in range(model_start_point + 1, self.args['num_iterations'] + 1):
            memory = []
            ...
            torch.save(self.model.state_dict(), f"model_p_{iteration}_{self.game}.pt")
            torch.save(self.optimizer.state_dict(), f"optimizer_p_{iteration}_{self.game}.pt")
...
class SPG:
    ...
    def update_root(self, prev_action, policy = None):
        if prev_action == None:
            self.root = Node(self.game, self.args, self.game.get_initial_state(), visit_count= 1)
            return
        else:
...
model_args = {
    ...
    'ConnectFour': (ConnectFour(), 9, 128, torch.device("cuda"))
}
...
if type(game) == TicTacToe:
    ...
    alphaZ = AlphaZeroParallel(model, optimizer, game, args)
...
if type(game) == ConnectFour:
    ...
    # model_start_point = 2
    # model.load_state_dict(torch.load(f"model_{model_start_point}_{game}.pt"))
    # optimizer.load_state_dict(torch.load(f"optimizer_{model_start_point}_{game}.pt"))
...
while True:
    ...
    if player == 1:
        ...
        mcts_probs = mcts.search(neutral_state, action)
    ...
    mcts.update_root(prev_action= action)
