...
ul = lambda l, i, e: l.__setitem__(i, e); aof = 4; fsz = 5; isz = 3; rns = ['sword', 'monster', 'empty']; inv = []; plm = [['?????' for i in range(fsz)] for j in range(aof)]; curf = aof - 1; curr = 0; r = 'empty'; go = False
...
def cmf(pusi, fs):
    mf = []
    for i in range(fs):
        f = cf(ri(0, fsz - 2), pusi, fsz); pusi = f.index("stairs up"); mf.append(f)
    return mf
def cl2f(pusi): stlf = cf(ri(2, fsz - 2), pusi, fsz); pusi = stlf.index("stairs up"); lf = cf(0, pusi - 1, fsz - 1); lf[0] = 'prize'; lf.insert(1, 'boss monster'); return (stlf, lf)
def rm(aof):
    m = []; frs = cff(); pusi = frs.index("stairs up"); m.append(frs)
    if aof - 3 > 0:
        mf = cmf(pusi, aof - 3); pusi = mf[-1].index("stairs up")
        for f in mf: m.append(f)
    stlf, lf = cl2f(pusi); m.append(stlf); m.append(lf); fi = 0; rmi = 0
    while m[fi][rmi] in ['prize', 'boss monster', 'stairs up', 'stairs down'] or (fi == 0 and rmi == 0): fi = ri(0, aof - 1); rmi = ri(0, fsz - 1)
    m[fi][rmi] = 'magic stones'
    while m[fi][rmi] in ['prize', 'boss monster', 'stairs up', 'stairs down', 'magic stones'] or (fi == 0 and rmi == 0): fi = ri(0, aof - 1); rmi = ri(0, fsz - 1); 
    m[fi][rmi] = 'exit gate'; return m[::-1]
